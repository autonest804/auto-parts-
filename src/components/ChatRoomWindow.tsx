...
import { ArrowLeft, Send, Phone, MapPin, ShoppingBag, Check, CheckCheck } from "lucide-react";
import { User, Chat, Message } from "../types";
import { subscribeToChatMessages, sendChatMessage, markMessagesAsRead } from "../lib/firebase";
                ...
                <div className="flex items-center gap-1 mt-1 px-1">
                  ...
                  {isMe && (
                    <span className="shrink-0">
                      {msg.status === "read" ? (
                        <CheckCheck size={11} className="text-sky-500 font-black" />
                      ) : msg.status === "delivered" ? (
                        <CheckCheck size={11} className="text-slate-400 font-black" />
                      ) : (
                        <Check size={11} className="text-slate-400 font-black" />
                      )}
                    </span>
                  )}
                </div>
