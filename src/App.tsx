...
export default function App() {
  ...
  return (
    <div className="h-screen w-screen overflow-hidden bg-slate-50 text-slate-900 flex flex-col font-sans" id="app-root">
      {authLoading ? (
        // Loading Splash screen
        <div className="flex-1 flex flex-col items-center justify-center bg-indigo-600 text-white animate-fade-in" id="splash-screen">
          <div className="flex flex-col items-center justify-center text-center">
            {/* White Auto Parts Logo */}
            <div className="w-24 h-24 bg-white/10 border border-white/20 backdrop-blur-md rounded-3xl flex items-center justify-center shadow-2xl mb-6 relative">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="w-12 h-12 text-white">
                <path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/>
              </svg>
            </div>
            <h1 className="text-4xl font-black tracking-wider text-white uppercase drop-shadow-sm">
              Auto Parts
            </h1>
            <p className="text-white/60 text-xs font-semibold tracking-widest uppercase mt-2">
              Premium Marketplace
            </p>
          </div>
        ...
        <div className="flex-1 flex flex-col bg-slate-50 relative h-full overflow-hidden select-none" id="app-shell">
          ...
          <nav className="h-20 bg-white border-t border-slate-100 flex items-center justify-around px-2 pb-2 sticky bottom-0 z-20 shadow-[0_-8px_30px_rgb(0,0,0,0.03)] select-none">
            {/* Home Tab */}
            <button
              onClick={() => setActiveTab("home")}
              className="flex-1 flex flex-col items-center justify-center py-1.5 relative cursor-pointer"
              id="nav-tab-home"
            >
              <div className="relative py-1 px-5 rounded-full transition-colors flex items-center justify-center">
                {activeTab === "home" && (
                  <motion.div
                    layoutId="activeTabIndicator"
                    className="absolute inset-0 bg-indigo-50 rounded-full animate-fade-in"
                    transition={{ type: "spring", stiffness: 380, damping: 30 }}
                  />
                )}
                <HomeIcon size={20} className={`relative z-10 transition-colors ${activeTab === "home" ? "text-indigo-600 scale-105" : "text-slate-400 hover:text-slate-500"}`} />
              </div>
              <span className={`text-[10px] font-black tracking-tight mt-1 transition-all ${
                activeTab === "home" ? "text-indigo-600 scale-105" : "text-slate-400"
              }`}>
                Home
              </span>
            </button>
            ...
            >
              <div className="relative py-1 px-5 rounded-full transition-colors flex items-center justify-center">
                ...
                <div className="relative z-10">
                  <MessageSquare size={20} className={`transition-colors ${activeTab === "chats" ? "text-indigo-600 scale-105" : "text-slate-400 hover:text-slate-500"}`} />
                  {(Object.values(unreadCounts) as number[]).reduce((sum, count) => sum + count, 0) > 0 && (
                    <span className="absolute -top-1.5 -right-2 bg-rose-600 text-[8px] text-white font-black h-4.5 w-4.5 rounded-full flex items-center justify-center border-2 border-white animate-pulse">
              ...
              <span className={`text-[10px] font-black tracking-tight mt-1 transition-all ${
                activeTab === "chats" ? "text-indigo-600 scale-105" : "text-slate-400"
              }`}>
                Chat
              </span>
            </button>
            ...
            <div className="flex-1 flex flex-col items-center justify-center relative">
              <div className="absolute -top-5 flex flex-col items-center">
                <button
                  ...
                  className={`h-13 w-13 rounded-full bg-gradient-to-tr from-indigo-600 to-indigo-500 text-white flex items-center justify-center shadow-lg border-4 border-white hover:from-indigo-500 hover:to-indigo-400 transform transition-all hover:scale-110 active:scale-95 cursor-pointer z-30 ${
                    activeTab === "sell" ? "shadow-indigo-500/20" : ""
                  }`}
                  id="nav-tab-sell"
                >
                  <PlusCircle size={24} className="text-white" />
                </button>
                <span className={`text-[10px] font-black tracking-tight mt-1.5 transition-all ${
                  activeTab === "sell" ? "text-indigo-600 font-extrabold scale-105" : "text-slate-400"
                }`}>
                  Sell
                </span>
              </div>
            ...
            >
              <div className="relative py-1 px-5 rounded-full transition-colors flex items-center justify-center">
                ...
                <Tag size={20} className={`relative z-10 transition-colors ${activeTab === "myads" ? "text-indigo-600 scale-105" : "text-slate-400 hover:text-slate-500"}`} />
              </div>
              <span className={`text-[10px] font-black tracking-tight mt-1 transition-all ${
                activeTab === "myads" ? "text-indigo-600 scale-105" : "text-slate-400"
              }`}>
                My Ads
              </span>
            </button>
            ...
            >
              <div className="relative py-1 px-5 rounded-full transition-colors flex items-center justify-center">
                ...
                <UserIcon size={20} className={`relative z-10 transition-colors ${activeTab === "account" ? "text-indigo-600 scale-105" : "text-slate-400 hover:text-slate-500"}`} />
              </div>
              <span className={`text-[10px] font-black tracking-tight mt-1 transition-all ${
                activeTab === "account" ? "text-indigo-600 scale-105" : "text-slate-400"
              }`}>
                Account
              </span>
            </button>
