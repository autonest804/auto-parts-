...
import { SparePart, User, Chat, Message, SellerReview, Notification, CAR_PART_CATEGORIES, INDIAN_CAR_BRANDS, POPULAR_LOCATIONS } from "../types";
...
export async function fetchMetadataConfig(): Promise<{
  ...
  if (useFirebase && db) {
    try {
      ...
      const brandSnap = await getDoc(doc(db, "config", "brands"));
      const locSnap = await getDoc(doc(db, "config", "locations"));

      return {
        categories: catSnap.exists() ? catSnap.data().list : CAR_PART_CATEGORIES,
        brands: brandSnap.exists() ? brandSnap.data().map : INDIAN_CAR_BRANDS,
        locations: locSnap.exists() ? locSnap.data().list : POPULAR_LOCATIONS
      };
    } catch (e) {
      console.warn("Firestore fetchMetadataConfig failed, using defaults:", e);
    }
  }

  // Fallback / LocalStorage
  const catRaw = localStorage.getItem("config_categories");
  const brandRaw = localStorage.getItem("config_brands");
  const locRaw = localStorage.getItem("config_locations");

  return {
    categories: catRaw ? JSON.parse(catRaw) : CAR_PART_CATEGORIES,
    brands: brandRaw ? JSON.parse(brandRaw) : INDIAN_CAR_BRANDS,
    locations: locRaw ? JSON.parse(locRaw) : POPULAR_LOCATIONS
  };
}

export async function saveMetadataConfig(type: "categories" | "brands" | "locations", data: any): Promise<void> {
  if (useFirebase && db) {
    try {
      ...
      console.warn(`Firestore saveMetadataConfig failed for ${type}:`, err);
      throw err;
    }
  }
  ...
  // Fallback
  if (type === "brands") {
    localStorage.setItem("config_brands", JSON.stringify(data.map));
  } else if (type === "categories") {
    localStorage.setItem("config_categories", JSON.stringify(data.list));
  } else {
    localStorage.setItem("config_locations", JSON.stringify(data.list));
  }
...
export async function sendAnnouncement(title: string, text: string): Promise<void> {
  ...
  anns.unshift({ title, text, createdAt: Date.now() });
